...
export default function Hero({ isPreloaded = false }) {
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden" ref={heroRef}>
      ...
      <div className="absolute z-10 bottom-24 md:bottom-[calc(var(--space-3xl)+32px)] left-7 right-7 md:left-14 md:right-14 flex flex-col md:flex-row items-start md:items-end justify-between gap-5 md:gap-6">
        <div className="flex flex-col items-start gap-2.5 max-w-[620px]">
          <div className="hero-title flex flex-wrap gap-1.5 md:gap-2 items-baseline font-['Clash_Grotesk_Variable',Georgia,serif] text-[clamp(1.9rem,6vw,3.4rem)] font-medium tracking-[-1px] md:tracking-[-1.5px] uppercase text-white leading-[1.08]">
          ...
          <p className="hero-sub text-sm sm:text-base md:text-[clamp(0.95rem,1.4vw,1.1rem)] font-normal text-white/80 leading-[1.5] md:leading-[1.6] tracking-[-0.01em] font-['Nimbus_Sans',sans-serif] opacity-0">
        ...
        <div className="hero-cta flex flex-col gap-3 w-full sm:w-[260px] self-start md:self-end mt-2 md:mt-0 opacity-0">
          <div
            className="w-full"
